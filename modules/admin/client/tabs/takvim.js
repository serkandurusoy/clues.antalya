...
Template.tabTakvimForm.helpers({
  tarihler: function() {
    var tarihler = [];
    for (var g=0; g<90; g++) {
      var tarih = moment(new Date(TimeSync.serverTime(null,10*1000))).add(g,'days');
      tarihler.push({
...
Template.tabTakvimList.helpers({
  olaylar: function() {
    var tarih = moment(new Date(TimeSync.serverTime(null,10*1000))).format('YYYY-MM-DD');
    return Rezervasyon.find({
      ...
      },
      {
        ...
        rez.tarih < moment(new Date(TimeSync.serverTime(null,10*1000))).format('YYYY-MM-DD') || rez.durum === 'tadilat' ? rez.gecmis = 'gecmis' : rez.gecmis = false;
